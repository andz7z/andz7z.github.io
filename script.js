...
(function(){
  const WEBAPP_URL = "https://script.google.com/macros/s/AKfycbyA5mmzIE-A9TXtiTk4sTvTm4eHAXy8F7FPHJb1P9HQFavK5B79I-8vS92hJx8t2anVnw/exec"; // -> înlocuieste
  const API_KEY = "AKfycbyA5mmzIE-A9TXtiTk4sTvTm4e"; // -> înlocuieste
  ...
  function renderStars(v){
    ...
    ratingStars.dataset.rating = v;
  }
  stars.forEach(s => s.addEventListener("click", () => { rating = Number(s.dataset.value); renderStars(rating); }));
  renderStars(rating);

  const elName = document.getElementById("rv-name");
  const elEmail = document.getElementById("rv-email");
  const elTitle = document.getElementById("rv-title");
  const elContent = document.getElementById("rv-content");
  ...
  async function fetchReviews(){
    try {
      const url = ${WEBAPP_URL}?action=list&limit=10&key=${API_KEY};
      const res = await fetch(url);
      const json = await res.json();
      if (!json.ok) { console.warn(json); return; }
  ...
  function renderReviews(arr){
    list.innerHTML = "";
    if (!arr.length) { list.innerHTML = "<p>Nu există încă review-uri.</p>"; return; }
    arr.forEach(r => {
      const div = document.createElement("div");
      div.className = "review-item";
      div.innerHTML = 
        <div class="review-header">
          <div><span class="review-name">${escapeHtml(r.name)}</span> · <span class="review-title">${escapeHtml(r.title||"")}</span></div>
          <div class="review-rating">${"★".repeat(Math.max(1,r.rating))}</div>
        </div>
        <div class="review-content">${nl2br(escapeHtml(r.content))}</div>
        <div class="review-meta">${new Date(r.timestamp).toLocaleString()}</div>
      ;
  ...
  btn.addEventListener("click", async () => {
    ...
    if (!payload.content || payload.content.length < 3) {
      msg.textContent = "Scrie un review (minim 3 caractere).";
      btn.disabled = false;
      return;
    }
    ...
    try {
      ...
      const json = await res.json();
      if (json.ok) {
        msg.textContent = "Mulțumim! Review trimis — va apărea după moderare.";
        elContent.value = ""; elTitle.value = "";
        // opțional re-fetch: fetchReviews();
      } else {
        msg.textContent = "Eroare trimitere: " + (json.error  json.message  "unknown");
      }
    } catch (err) {
