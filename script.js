/* Main JS: Three.js Starfield + Loader transition + Hero text animation */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      ...
      // arată aplicația și pornește stelele
      app.classList.remove('hidden');
      initStarfield();
      animate();

      // fade-in app + text slogan
      setTimeout(() => {
        app.classList.add('active');

        // animăm fiecare cuvânt din slogan
        const words = document.querySelectorAll('.hero-text .word');
        words.forEach((word, i) => {
          word.style.animationDelay = `${0.4 + i * 0.4}s`;
          word.style.animationPlayState = 'running';
        });

      }, 300); // mic delay după ce app devine vizibil

    }, 900); // durata tranziției loader-ului
  }, 3000); // durata loader-ului (3 secunde)
